...
- **Brackets** for every classification with the correct format:
  ...
  |-------|-------|---------|------------------|-------|-------|
  | 6A | Public | 4 | 6 | 24 | Top 2 seeds per region get a first-round bye |
...
- Each slot shows the **team name (left)** and **score (right)**. Click a game
  for a pop-up with date, time, location, both teams' overall & region records,
  and ratings.
- **Projected results** toggle on every bracket — fills undecided games from the
  ratings index.
- **Region standings** tab: teams in seeded order with overall record, region
  record, rating, and a colored **Status** pill (Clinched / High / Medium /
  Low / Out). Per-region notes appear under each table.
...
- **Standings & Seeding** — drag the `⠿` handle to reorder teams (order = seed;
  top N qualify and feed the bracket). Edit name, overall/region records, rating,
  and status inline. Add/remove teams. Write per-region notes.
